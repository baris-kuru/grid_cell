...
n_seeds = len(npzfiles)
th = np.vstack((rate_th_sim, rate_th_diff, phase_th_sim, phase_th_diff, complex_th_sim, complex_th_diff)).reshape(6*n_seeds)
speed = 1/th
hue = (['similar']*n_seeds+ ['distinct']*n_seeds)*3
code = ['rate']*n_seeds*2 +['phase']*n_seeds*2+['complex']*n_seeds*2


df = pd.DataFrame({'threshold cross': th,
...
plt.title('Threshold Crossing in 2000ms')
...
plt.close('all')
ax1 = sns.barplot(x='code', y='threshold cross', hue='trajectories', data=df, order=['rate', 'phase', 'complex'])
plt.title('Threshold Crossing in 2000ms')
parameters = ('lr = 5*$10^{-4}$,  9 Grid seeds, 5 Poisson seeds,'+ 
              '  $threshold cross$ = number of epochs until RMSE reached a threshold of 0.2')
plt.annotate(parameters, (0,0), (0, -35), xycoords='axes fraction', textcoords='offset points', va='top', fontsize=12)
...
for idx, val  in enumerate(rate_th_sim):
    loc = width/2
    rate_sim = val[0]
    rate_diff = rate_th_diff[idx][0]
    phase_sim = phase_th_sim[idx][0]
    phase_diff = phase_th_diff[idx][0]
    complex_sim = complex_th_sim[idx][0]
    complex_diff = complex_th_diff[idx][0]
