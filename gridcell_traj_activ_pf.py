...
def grid_population(n_grid, max_rate, seed, arr_size=200):
    ...
    for i in range(n_grid):
        x = grid_phase[i][0]
        y = grid_phase[i][1]
        arr = grid_maker(grid_spc[i], grid_ori[i], [x, y], arr_size, [1,1], max_rate, seed_1)
        all_grids[:, :, i] = arr
    mean_grid = np.mean(all_grids, axis=2)
    
    return all_grids, mean_grid
    

def draw_traj(all_grids ,n_grid, n_traj, arr_size=200, field_size_cm=100, speed_cm=20):
    ...
    dur_s = int(field_size_cm/speed_cm)
    dur_ms = dur_s*1000
    start_cm = 75
    dt_s = (dur_s)/arr_size #for parallel one
    ...
    par_idc = np.array([149, 148, 147, 146])
    par_idc_cm = ((par_idc+1)/size2cm)
    n_traj = 4
    ...
    # cum_tilt = np.sum(tilt_traj, axis=1)*(dt_s)
    
    return par_traj, par_idc_cm, dur_ms, dt_s
