...
def rate_only_spike_ct(trajs_pf, dur_ms, seed_2s, n_traj=2):
    ...
    for idx, seed_2 in enumerate(seed_2s):
        ...
        granule_out = pyDentate(curr_spikes, seed_2, n_traj, dur_ms, pp_weight)[0]
...
def phase_code(trajs, dur_ms, seed_1, seed_2s, pp_weight, f=10, shift_deg=240):
    dur_s = dur_ms/1000
    T = 1/f
    bin_size_ms = T*1000
    n_phase_bins = 360
...
def pyDentate(input_grid_out, seed_1, seed_2, n_traj, dur_ms, pp_weight):
    savedir = os.getcwd()
    input_scale = 1000
    seed_3 = seed_1+150 #seed_3 for network generation & simulation
    ...
    for trj in range(n_traj):
        nw = net_tunedrev.TunedNetwork(seed_3, input_grid_out[:,trj], PP_to_GCs, PP_to_BCs, pp_weight=pp_weight)
        ...
        granule_output[:,trj] =  copy.deepcopy(np.array([cell[0].as_numpy() for cell in nw.populations[0].ap_counters]))
        mossy_output[:,trj] =  copy.deepcopy(np.array([cell[0].as_numpy() for cell in nw.populations[1].ap_counters]))
        basket_output[:,trj] =  copy.deepcopy(np.array([cell[0].as_numpy() for cell in nw.populations[2].ap_counters]))
        hipp_output[:,trj] =  copy.deepcopy(np.array([cell[0].as_numpy() for cell in nw.populations[3].ap_counters]))
        
        fig = nw.plot_aps(time=dur_ms)
        tuned_fig_file_name = (str(nw) + "spike_plot_rate_n_phase_gra_out_for_perceptron_seed3_dur_weight_"+
                    str(seed_3)+ '_' + str(dur_ms) +'_' +str(pp_weight)+'_'+ str(trj))
        nw.save_ap_fig(fig, savedir, tuned_fig_file_name)
    
    path = '/home/baris/repo/pyDentate_results/'
    fname = 'pyDentate_out_dur_'+str(dur_ms)+'_weight_'+str(pp_weight)+'_seed1_'+str(seed_1)+'_seed2_'+str(seed_2)
    np.savez(path+fname, 
